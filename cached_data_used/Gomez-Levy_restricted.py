...
def tune(device_name, strategy="bayes_opt", strategy_options=None, verbose=False, quiet=False, simulation_mode=True):
    ...
    tune_params["x"] = np.linspace(-1, 0.75, num=100).tolist()
    tune_params["y"] = np.linspace(-1, 1, num=100).tolist()
    restrict = ["-np.sin(4 * np.pi * x) + 2 * np.sin(2 * np.pi * y)**2 <= 1.5"]
