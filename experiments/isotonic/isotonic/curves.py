...
class AbstractIsotonicCurve(metaclass=abc.ABCMeta):
    def __init__(self, x, y, increasing=True):
        ...
        assert (np.all(x[1:] >= x[0:-1])), "Each x-value must be larger or equal to the previous one."
        self.x = x
    ...
    def __check_monotonic(self, x, varname, increasing):
        if increasing:
            if (np.all(x[:-1] >= x[1:])):
                raise ValueError("Input " + varname + " was not monotonically non-decreasing, but should have been.")
        else:
            if (np.all(x[:-1] <= x[1:])):
                raise ValueError("Input " + varname + " was not monotonically non-increasing, but should have been.")
